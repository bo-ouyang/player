...
function getTransactionByHash(hash,callback) {
	var query = `module=proxy&action=eth_getTransactionByHash&txhash=${hash}&apikey=${apikey}`
...
function txexist(hash, callback){
  var query = `module=localchk&action=txexist&txhash=${hash}&apikey=${apikey}`
  request(url + query,function(err,response,res){
		res = JSON.parse(res)
		callback(res.result)
	})
...
function gettxreceiptstatus(hash, callback) {
  var query = `module=transaction&action=gettxreceiptstatus&txhash=${hash}&apikey=${apikey}`
  request(url + query,function(err,response,res){
  		var result = {}
		try{
			res = JSON.parse(res)
...
function balance(address, callback){
  var query = `module=account&action=balance&tag=latest&address=${address}&apikey=${apikey}`
...
function balances(addresses, callback) {
  var query = `module=account&action=balancemulti&tag=latest&address=${addresses}&apikey=${apikey}`
  request(url + query,function(err,response,res){
  		var result = []
		try{
			res = JSON.parse(res)
...
function txlist(address,p,offset,callback){
  var query = `module=account&action=txlist&page=${p}&offset=${offset}&sort=desc&address=${address}&apikey=${apikey}`
  request(url + query,function(err,response,res){
  		var result = []
		try{
			res = JSON.parse(res)
...
function getabi(tokenaddress,callback){
  	var query = `module=contract&action=getabi&address=${tokenaddress}&apikey=${apikey}`
  	request(url + query,function(err,response,res){
  		var abi = null
		try{
			res = JSON.parse(res)
...
function getTxList(walletAddr, startblock, callback) {
	var query = `module=account&action=tokentx&address=${walletAddr}&startblock=${startblock}&endblock=999999999&sort=asc&apikey=${apikey}`;
  	request(url + query, function(err, response, res) {
  		var result = [];
