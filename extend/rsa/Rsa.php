...
class Rsa
{
     ...
     */
    function __construct($pubKey = null, $priKey = null)
    {
        if (is_null($pubKey)) {
        ...
        if (is_null($priKey)) {
            $priKey = Env::get('extend_path') . 'rsa/private.key';
        }

        $publicKey  = file_exists($pubKey) ? file_get_contents($pubKey) : (is_null($pubKey) ? null : $pubKey);
        $privateKey = file_exists($priKey) ? file_get_contents($priKey) : (is_null($priKey) ? null : $priKey);
    ...
    {
        $key = $pem = chunk_split($this->publicKey, 64, "\n");
